...
export function AssignmentManager({
  ...
  const lessonsLoading = useSelector(
    (state: RootState) => state.lessons.loading
  );
  ...
  const totalPoints = lessons.reduce(
    (acc, l) => acc + (l.completed ? l.rewards.points : 0),
    0
  );
  const totalXP = lessons.reduce(
    (acc, l) => acc + (l.completed ? l.rewards.xp : 0),
    0
  );
  ...
  const getRecommendedLesson = () => {
    ...
    if (userProgressData && userProgressData.attempts.length > 0) {
      const recommendedDifficulty = AdaptiveAI.calculateNextDifficulty(
        userProgressData.attempts
      );
      return (
        uncompletedLessons.find(
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4 text-sm opacity-75">
                <span>
                  Vence: {new Date(group.dueDate).toLocaleDateString()}
                </span>
                <span>•</span>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-blue-600">
                          {getRecommendedLesson()?.estimatedDuration} min •{" "}
                          {getDifficultyLabel(
                            getRecommendedLesson()?.difficulty || 1
                          )}
                        </p>
                      </div>
                      <Button
                      ...
                      <div className="flex-1">
                        ...
                        {lesson.userProgress && (
                          <div className="mt-2 text-xs text-muted-foreground">
                            ...
                            Intentos: {lesson.userProgress.attempts} • Racha:{" "}
                            {lesson.userProgress.currentStreak}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-green-600">
                          Tu rendimiento ha sido excelente. La IA sugiere
                          aumentar la dificultad para mantener el desafío.
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-blue-600">
                          Basado en tu patrón de aprendizaje, sesiones de 15-20
                          minutos son más efectivas para ti.
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-orange-600">
                          Incluir más problemas de aplicación práctica mejorará
                          tu comprensión conceptual.
                        </p>
                      </div>
                    </div>
                  </div>
